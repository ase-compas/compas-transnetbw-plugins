...
export class DOTypeV2Service implements IDoTypeV2Service {
  ...
  async getReferencedTypes(id: string, childNameFilter?: ChildNameFilter): Promise<DataTypes> {
    const referencedTypes =  this.typeRepo.findReferencedTypesById(DataTypeKind.DOType, id, childNameFilter);
    if (!referencedTypes) {
  ...
  async getDefaultType(cdc: string): Promise<DOTypeDetailsV2> {
    const children: ObjectReferenceDetails[] = await this.dataTypeService.getDefaultObjectReferenceDetails(DataTypeKind.LNodeType, cdc);
