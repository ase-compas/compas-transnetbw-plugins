...
export function openTypeDetailsDrawer(typeId: string, typeKind: TypeKind, service: DataTypeService, docState: DocState, mode: ViewMode = 'view') {
  return openDrawer({
    title: typeKind ? `${TypeKind.abbreviation(typeKind)}[${typeId}]` : typeId,
    component: TypeDetails,
    props: { typeId, service, docState, mode },
  });
}
