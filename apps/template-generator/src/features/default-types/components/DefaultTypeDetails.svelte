<script lang="ts">
  ...
  import { DefaultTypeDetailsState } from '../state/default-type-details.state.svelte';
  ...
  async function saveDefaultType() {
    ...
    const result = await openDialog(SaveDefaultTypeDialog, {
      mode: mode,
      initialVersion: detailsState.isCreateMode ? detailsState.info.version: undefined
    });
    ...
    await detailsState.saveAsNewVersion(saveInfo);
    syncDocToRuntime();
  }
  ...
  async function loadVersion(version: Version)  {
    ...
    await detailsState.loadByVersion(version.version);
    syncDocToRuntime();
  ...
  onMount(async () => {
    ...
    if (id) {
      ...
      await detailsState.loadById(id);
      syncDocToRuntime();
    } else if (createInfo) {
      setupCreateMode();
    } else {
    ...
    <div class="default-type__header">
      <div class="default-type__header-left">
          ...
          <div class="default-type__version_info description">
            ...
            <Textfield value={detailsState.info.description ? detailsState.info.description : 'No description'} disabled variant="outlined" />
          </div>
      </div>
