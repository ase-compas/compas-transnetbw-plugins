...
import { TypeRenameDialog } from "@oscd-transnet-plugins/oscd-template-generator";
import { TypeKind, type SimpleDataType } from "./model";
import { getDataTypeService, type DataTypeService } from "./type.service";
import CreateTypeDialog from "./CreateTypeDialog.svelte";
...
export async function renameDataTypeWorkflow(
    ...
    const result = await openDialog(TypeRenameDialog, {
        typeId: dataType.id,
        typeKind: TypeKind.toDataTypeKind(dataType.typeKind),
    });
