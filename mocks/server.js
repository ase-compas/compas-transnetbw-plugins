...
app.post('/compas-scl-data-service/api/locations', (req, res) => {
  return res.json({ uuid: 'f1b2c3d4-e5f6-7890-1234-56789abcdff1' , ...req.body});
})
...
app.post('/compas-scl-data-service/api/archive/resources/search', (req, res) => {
  const { uuid, location, name, approver, contentType, type, voltage, modifiedAt, archivedAt } = req.body;
  ...
  const filteredResources = mockArchivedResources.filter((item) => {
    ...
    if (approver && item.aprover !== approver) {
      filter = false;
    }
