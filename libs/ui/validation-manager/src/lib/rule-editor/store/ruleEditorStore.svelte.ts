...
} from '../utils/validationRuleUi';

function defaultEntry(): RuleDraft {
  return {
    title: '',
    description: '',
    context: '//SCL',
...
/**
 * Editor state for the rule wizard.
 *
 * A module level singleton is deliberate: the wizard is a modal flow, so at
 * most one rule is ever being edited at a time. {@link initRuleEditor} resets
 * it whenever a new editing session starts.
 */
...
/** Loads a draft into the editor, or resets it for a fresh rule. */
export function initRuleEditor(existing?: RuleDraft): void {
  if (existing) {
    validationEditor.entry = { ...existing };
    validationEditor.ruleUi = existing.ruleUi
      ? restoreRuleUi(existing.ruleUi)
      : parseAssertionToRuleUi(existing.assertion, existing.message ?? '');
  } else {
    validationEditor.entry = defaultEntry();
